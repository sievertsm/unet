...
def check_loss(model, loader, device='cpu', dtype=torch.float, ltype=torch.long):
    '''
    ...
    with torch.no_grad():
        for b, (x, y) in enumerate(loader):
            ...
            loss = F.cross_entropy(scores, y).item()
...
def display_images(images, titles=['Image', 'Mask', 'Network Output'], cmap='bone', height=10, save_path=None, show=True):
    '''
...
def train(model, optimizer, loader_train, loader_val, epochs=1, print_every=5, device='cpu', dtype=torch.float, ltype=torch.long, early_stop=None, save_results=False, display_input=None):
    '''
    ...
    notes             -- dictionary of notes to be saved as a txt file
    start_epoch       -- epoch number to start to continue training
    ...
    for e in range(epochs):
        ...
        l_trn = check_loss(model, loader_train, device=device, dtype=dtype, ltype=ltype)
        loss_train.append(l_trn)
        
        # check validation loss
        l_val = check_loss(model, loader_val, device=device, dtype=dtype, ltype=ltype)
